...
export default () => {
  ...
  const fetchPost = async (values) => {
    ...
    try {
      const response = await api.patch(`/api/post/${post._id}`, {
      ...
      <div>
        <div className="container mx-auto flex justify-center py-10">
          <Formik 
            enableReinitialize
            initialValues={{
              title_image: post ? post.title_image : "",
              tagList: post ? post.tagList.map((element ) => ({label:element , value:element})) : "",
            ...
            validationSchema={
              Yup.object().shape({
                title_image: Yup.string().min(6, 'Titulo do post precisa ter no minimo 6 caracteres'),
                post_title: Yup.string().min(6, 'Titulo do post precisa ter no minimo 6 caracteres').max(40, 'Titulo do post precisa ter no maximo 40 caracteres'),
                about: Yup.string().min(6, 'Introdução do post precisa ter pelo menos 6 caracteres').max(500, 'Calma ai a introdução excedeu o limite de caracteres'),
                content: Yup.string().min(10, 'Post precisa ter no minimo 10 caracteres'),
                tagList: Yup.array().min(1, ''),
              })}
          >
            {({
              values,
              ...
              setFieldValue,
                ...
                <div className="bg-white shadow-lg rounded-lg w-8/12 box-border p-8 h-max-content">
                  <p className="text-center mb-8 font-semibold text-2xl">Editar Post </p>
                  ...
                  <input type="submit" onClick={handleSubmit} disabled={isSubmitting} className="p-4 w-full text-center text-white bg-success-500 rounded-lg text-lg box-border hover:bg-success-600 font-bold" value="Editar Post" />
                </div>
              )}
          </Formik>
        </div>
